...
let allCaps = [];
runners.map(cap => allCaps.push("First Name: " + cap.first_name.toUpperCase()));
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
let isItLarge = runners.filter((large) => {
    return large.shirt_size === "L";
});
largeShirts.push(isItLarge);
console.log(isItLarge);
...
const total = runners.reduce((total, num) => {return total += num.donation}, 0);
...
// Problem 1
//Kayla got married. Change Kayla Whitwam's last name to Williams.
let newRunners = runners.filter((name) => {
    return runners.last_name !== "Whitwam";
});
let getName = runners.filter((name) => {
    return name.last_name === "Whitwam";
});
getName[0].last_name = "Williams";
newRunners.push(getName);
console.log(newRunners);
// Problem 2
//need to know how all the emails of the runners.
let emails = [];
runners.forEach(name => emails.push(name.first_name + ' ' + name.last_name + ': ' + name.email));
console.log(emails);
